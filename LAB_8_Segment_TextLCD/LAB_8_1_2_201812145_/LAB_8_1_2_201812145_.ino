...
int digit_select_pin[] = { 67, 68, 69}; // 자릿수 선택 핀
// 7세그먼트 모듈 연결 핀 ‘a, b, c, d, e, f, g, dp’ 순서
int segment_pin[] = {58, 59, 60, 61, 62, 63, 64, 65};

int count = 0;
int SEGMENT_DELAY = 5; // 숫자 표시 사이의 시간 간격 - 잔상효과
...
void show_4_digit(int number) {
  ...
  //잔상효과를 이용해 4자리 숫자를 4자리 7세그먼트에 출력
...
void loop() {
  //잔상효과를 위해 빠른속도로 쓰기를 반복해야 하므로 delay함수를 사용할 수 없다
  time_current = millis();
